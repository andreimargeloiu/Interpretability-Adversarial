...
from .tools import helpers, one_cycle
...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    if str.lower(args.optimizer)=='sgd':
        optimizer = SGD(param_list, args.lr, momentum=args.momentum,
                                    weight_decay=args.weight_decay)
    elif str.lower(args.optimizer)=='adam':
        optimizer = Adam(param_list, args.lr, weight_decay=args.weight_decay)
    else:
        raise ValueError("Optimizer error invalid. Only Adam and SGD are supported")

    # Make schedule
    schedule = None
    if args.step_lr:
        schedule = lr_scheduler.StepLR(optimizer, step_size=args.step_lr)
    elif args.custom_schedule == 'plateau':
        schedule = lr_scheduler.ReduceLROnPlateau(optimizer, factor=0.1, patience=args.lr_patience, verbose=True)
    elif args.custom_schedule == 'cyclic':
        eps = args.epochs
        lr_func = lambda t: np.interp([t], [0, eps*2//5, eps], [0, args.lr, 0])[0]
        schedule = lr_scheduler.LambdaLR(optimizer, lr_func)
    elif isinstance(args.custom_schedule, dict) and args.custom_schedule['policy'] == 'one_cycle':    
        schedule = one_cycle.OneCycleLR(optimizer, 
            args.custom_schedule['total_steps'],
            lr_range=(args.custom_schedule['min_lr'], args.custom_schedule['max_lr']))

    elif args.custom_schedule is not None:
...
def eval_model(args, model, loader, store):
    ...
    """
    # set the model to return both prediction and image
    model.module.only_prediction = False
    ...
    prec1, nat_loss = _model_loop(args, 'val', loader, 
                                        model, None, None, 0, False, writer)
...
def train_model(args, model, loaders, *, checkpoint=None, 
    ...
    Args:
        args (object) : A python object for arguments, implementing
            ...
            optimizer (str, *required*)
                the optimizer to used (Adam or SGD)
            ...
            batch_size (int, optional)
                The batch size used for training. (Used only for logging, and not in execution.)
            arch (str, optional)
                The model architecture type. (Used only for logging, and not in execution.)
            pytorch_pretrained (bool, optional)
                Whether the model is pretrained or not. (Used only for logging, and not in execution.)
            use_adv_prec (bool, optional)         
                If `True`, then use adversarial precision to define the best model. Else, use standard accuracy.
            do_tqdm(bool, optional)
                if `True`, then use tqdm. Else, print the last description on the screen.
    ...
    if store is not None: 
        ...
        store.add_table(consts.METADATA_TABLE, consts.METADATA_SCHEMA)
        
        repo = git.Repo(search_parent_directories=True)
        sha = repo.head.object.hexsha
        store[consts.METADATA_TABLE].append_row({
            'args': args,
            'sha': sha
        })
    ...
    best_loss, start_epoch = (1e7, 0)
    # if checkpoint:
    #     start_epoch = checkpoint['epoch']
    #     best_loss = checkpoint[f"{'adv' if args.adv_train else 'nat'}_loss"]

    ### I commented this line because it was raising an error while training in Colab.
    ...
    best_epoch = 0 # used for early stopping

    for epoch in range(start_epoch, args.epochs):
        # train for one epoch
        train_prec1, train_loss = _model_loop(args, 'train', train_loader, 
                model, opt, schedule, epoch, args.adv_train, writer)
        ...
        if should_log or last_epoch or should_save_ckpt:
            ...
            if args.adv_train and args.use_adv_prec:
                our_loss = adv_loss
            else:
                our_loss = nat_loss
            ...
            if epoch - best_epoch == args.es_patience:
            ...
            if is_best: 
                ...
                best_epoch = epoch
                print(f"Saving best checkpoint at epoch {best_epoch}")

        if schedule and not isinstance(schedule, one_cycle.OneCycleLR):
            if isinstance(schedule, lr_scheduler.ReduceLROnPlateau):
                if should_adv_eval:
                    schedule.step(adv_loss)
                else:
...
def find_lr(args, model, loader, start_lr=1e-7, end_lr=10, num_iter=10, criterion=nn.CrossEntropyLoss()):
    """
    Finds an appropriate learning rate, similar to fast.ai.

    Args:
    - model (AttackeModel)
    - loader: the training loader    
    """
    prev_only_prediction = model.only_prediction

    model.only_prediction = True

    optimizer, schedule = make_optimizer_and_schedule(
        args, model, checkpoint=None, params=None)

    lr_finder = LRFinder(model, optimizer, criterion, device=args.device)
    lr_finder.range_test(loader, start_lr=start_lr, end_lr=end_lr, num_iter=num_iter)
    lr_finder.plot()   # to inspect the loss-learning rate graph
    lr_finder.reset()  # to reset the model and optimizer to their initial state
    model.only_prediction = prev_only_prediction

    return lr_finder


def _model_loop(args, loop_type, loader, model, opt, schedule, epoch, adv, writer):
    """
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    if args.do_tqdm:
        iterator = tqdm(enumerate(loader), total=len(loader), position=0, leave=True)
    else:
    ...
    for i, (inp, target) in iterator:
        ...
        if is_train:
            ...
            loss.backward()
            opt.step()
            if schedule and isinstance(schedule, one_cycle.OneCycleLR):
                schedule.step()
