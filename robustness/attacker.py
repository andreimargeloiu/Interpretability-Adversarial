...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
        ...
        criterion = ch.nn.CrossEntropyLoss(reduction='none').to(self.device)
...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
                fake_relu=False, no_relu=False, with_prediction=True, **attacker_kwargs):
        """
        ...
        Parameters:
            ...
            with_prediction (bool) : if :samp:`False`, only return the model prediction
