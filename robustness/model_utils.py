...
def make_and_restore_model(*_, arch, dataset, resume_path=None,
         parallel=True, pytorch_pretrained=False, device='cpu'):
    """
    ...
    classifier_model = dataset.get_model(arch, pytorch_pretrained) if \
                            (isinstance(arch, str) or isinstance(arch, dict)) else arch
    ...
    if resume_path:
        if os.path.isfile(resume_path):
            ...
            checkpoint = ch.load(resume_path, pickle_module=dill, map_location=lambda storage, loc: storage)
